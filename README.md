...
| **Arkime** `:8005` | Pas de **PCAP complet** interrogeable : l'analyse tshark porte sur des fichiers déposés. Arkime capture en continu et indexe les sessions dans l'ES existant. | Bouton « Arkime » sur `/pcap-analysis`, « PCAP » sur `/ip/<ip>` (sessions filtrées sur l'adresse). Premier lancement : `make arkime-init`. Sans login en labo (viewer lié à `127.0.0.1`) ; en prod `ARKIME_AUTH_MODE=form` derrière le reverse-proxy HTTPS (compte initial `admin`/`admin`). |
