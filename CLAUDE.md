...
- **Nom du produit** : « NetWatch » n'est pas utilisable commercialement (société NETWATCH SAS à Paris en conseil informatique, netwatch.ai = supervision IT par IA, Netwatch Irlande = vidéosurveillance). Session de noms du 18/09 (~80 candidats, tout le vocabulaire gardien/muraille/sentinelle est déjà une marque de sécurité) → **candidat retenu : Teichos** (τεῖχος, la muraille ; *teichoscopie* = observer depuis les remparts ; produit/rapport possible : *Teichoscope*). Aucune collision trouvée sur le web. Reste à faire par Nicolas : INPI classes 9 et 42, TMview, domaines `teichos.fr` / `.io`. Renommage (dépôt, préfixes `netwatch-`, index `netwatch-*`, docs) = chantier à part, après la validation VM. Secours : Vedetta, Lotharia, Dunum, Rempart.
